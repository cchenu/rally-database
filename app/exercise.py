...
df_question1 = pd.DataFrame(
    list_question1, columns=["Last Name", "First Name", "Citizenship"]
)
df_question1 = df_question1.sort_values(
...
df_question3 = pd.DataFrame(
    list_question3,
    columns=["Number", "Name of the starting city", "Name of the ending city"],
)
...
st.divider()
st.header(
    "1) Lister par ordre alphabétique les participants du rallye Paris Dakar "
    "de l'année 2000 ayant appartenu à la catégorie moto."
)
st.divider()
st.write("")
...
with col1:
    st.write("")
    st.subheader(
        f"Le nombre d'étapes dont la ville de départ et la ville d'arrivée se "
...
st.header(
    "3) Lister la liste d'étapes (numéro, ville départ et ville d'arrivée) se "
    "déroulant au Sénégal."
)
st.divider()
st.write("")
...
with col2:
    st.dataframe(df_question3, width="stretch")
...
with col1:
    st.dataframe(df_question4, width="stretch")
...
with col2:
    st.write("")
    st.subheader(
        "Le nombre de véhicule appartenant à la marque Toyoa ayant participé "
        "au rally Paris-Dakar pour les 10 dernières années (depuis 2004) est "
        f"de {nombre_vehicule_toyota[0][0]}."
    )
...
with col2:
    st.dataframe(df_question7, width="stretch")
